...
struct ParseResult
{
	...
	ParseResult(IndexT _currPos, IndexT _lastPos )
	: myData(0)
	, myCurrPos(_currPos)
	, myLastPos(_lastPos)
	{}
	
	ParseResult(const Out& _data, IndexT _currPos, IndexT _lastPos )
	: myData(new Out(_data))
	, myCurrPos(_currPos)
	, myLastPos(_lastPos)
	{}

	Out& data() { return (*myData); } 
	bool input_consumed() { return myCurrPos == myLastPos; }
	
	bool parse_finished() { return myData && input_consumed(); } 
	IndexT parse_position() { return myCurrPos; }

private:
	IndexT myCurrPos;
	IndexT myLastPos;
	std::shared_ptr<Out> myData;
};
